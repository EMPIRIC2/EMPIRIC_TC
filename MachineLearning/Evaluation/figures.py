...
def plot_quantile_maps(models_statistics, save_path=None):
    ...
    fig, axes = plt.subplots(nrows=len(models_statistics), ncols=5, figsize=(13,5), layout='compressed', subplot_kw={'projection': ccrs.PlateCarree(central_longitude=180)})
...
def plot_example_site_boxplot(ground_outputs, models_outputs, n_examples, save_path=None):
    """
...
def make_collective_model_figures(all_outputs, all_statistics, save_folder):
    ...
    all_site_outputs = {model: get_many_site_values(outputs) for model, outputs in all_outputs.items()}

    example_site_ensemble_boxplot_figure(all_site_outputs,
                                         os.path.join(save_folder, "site_ensemble_boxplot.png"))
    ...
    plot_example_site_boxplot(ground_outputs, models_outputs, 10, os.path.join(save_folder, "example_site_boxplots.png"))

def make_single_model_figures(ground_outputs, model_outputs, ground_statistics, model_statistics, metrics, save_folder):
    """
