...
def compute_ensemble_statistics(outputs):
    statistics = {"Quantiles": get_quantiles(outputs), "Mean": np.mean(outputs, axis=0)}
...
def kolmogorov_smirnov_statistics(ground_truths, predictions):
    ...
    :param: ground_truths: the outputs from the model being used as the
    baseline (i.e. STORM) (a list of n x m numpy arrays)
    :param: predictions: outputs of the model being evaluated
    (a list of n x m numpy arrays of the same length as ground_truths)
    ...
    predictions = np.reshape(
        predictions, (len(ground_truths), ground_truth_sample_length)
    )

    # compute the kolmogorov smirnov statistic
    for i in range(len(ground_truths[0])):
        ks_statistics.append(
            scipy.stats.kstest(ground_truths[:, i], predictions[:, i]).statistic
        )
