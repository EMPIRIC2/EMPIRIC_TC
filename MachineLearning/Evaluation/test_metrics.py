...
from MachineLearning.Evaluation.evaluation_testing_utils import TEST_GRIDS_1, TEST_GRIDS_2, STATISTICS_2, STATISTICS_1, ALL_METRICS
...
class TestMetrics(unittest.TestCase):
    ...
    def test_ensemble_statistics_no_sites(self):
        ...
        self.assertEqual(
            unet_statistics["Quantiles"].shape,
            (5, 110, 210)
        )

        self.assertEqual(
            storm_statistics["Quantiles"][:, 55, 45].tolist(),
            [5, 6.5, 8.5, 12, 18]
        )
        self.assertEqual(
            unet_statistics["Quantiles"][:, 55, 45].tolist(),
            [10, 10, 10.5, 12.5, 17]
        )
