...
def upsample_block(filters, up_sample_size, kernel_size, batch_norm=True, dropout=False):
    ...
    result.add(Conv2D(filters = filters, kernel_size=kernel_size, padding='same', kernel_initializer='HeNormal', use_bias=False))
...
def downsample_block(filters, size, batch_norm=True, dropout=False):
    ...
    result = Sequential()
    result.add(
        Conv2D(filters, size, padding='same', kernel_initializer='HeNormal', activation=LeakyReLU(), use_bias=False))
...
def UNet(genesis_size, output_channels, dropout, batch_norm, down_filters, up_filters):

    genesis = Input(shape=genesis_size)
    skips = []

    x = Conv2D(64, (3,3), activation=LeakyReLU(), padding='same', kernel_initializer='HeNormal')(genesis)

    #down_filters = [64*2, 128*2, 256*2]
    for i, filters in enumerate(down_filters):

        skips.append(x)
        x = downsample_block(filters, (3,3), batch_norm=batch_norm, dropout=dropout)(x)
    ...
    for i, filters in enumerate(up_filters):
        ...
        x = upsample_block(filters, (2,2), (3,3), batch_norm=batch_norm, dropout=dropout)(x)
    ...
    output = Conv2D(output_channels, (1,1), kernel_initializer='HeNormal', use_bias=False)(x)
