...
def monthly_mean_pressure(data):
    ...
    data_1980_2018_monthly_means = (
        data.sel(time=slice("1980-01-01", "2018-01-01"))
        .groupby("time.month")
        .mean("time")
    )
...
def wind_pressure_relationship():
    ...
    data = xr.open_dataset(
        os.path.join(__location__, "SST_monthlyavg_ERA5_1959_2022.nc")
    )
...
def calculate_MPI_fields():
    ...
    mpi_bounds = [
        [860, 880, 900, 900, 880, 860],
        [920, 900, 900, 900, 880, 880],
        [840, 860, 880, 900, 880, 860],
        [840, 880, 860, 860, 840, 860],
        [840, 840, 860, 860, 840, 840],
        [860, 860, 860, 870, 870, 860, 860],
    ]
...
def pressure_coefficients():
    ...
    for idx in range(0, 6):
        ...
        for i in range(len(months[idx])):
            ...
            matrix_mean = -100 * np.ones(
                (int((lat1 - lat0) / 5), int((lon1 - lon0) / 5))
            )
            matrix_std = -100 * np.ones(
                (int((lat1 - lat0) / 5), int((lon1 - lon0) / 5))
            )
            ...
            for latidx in latbins:
                ...
                for lonidx in lonlist:
                    ...
                    for lat_sur in [-5, 0, 5]:
                        for lon_sur in [-5, 0, 5]:
                            if (int(latidx + lat_sur), int(lonidx + lon_sur)) in lijst:
                                if (
                                    np.nanmin(MPI[latidx + lat_sur][lonidx + lon_sur])
                                    > 0.0
                                ):
                                    for pr, d0, d1 in zip(
                                        Pressure[latidx + lat_sur][lonidx + lon_sur],
                                        DP0[latidx + lat_sur][lonidx + lon_sur],
                                        DP1[latidx + lat_sur][lonidx + lon_sur],
                                    ):
                                        preslist.append(pr)
                                        dp0list.append(d0)
                                        dp1list.append(d1)
                    ...
                    if len(preslist) > 9.0:
                        ...
                        try:
                            opt, l = curve_fit(
                                PRESFUNCTION, X, dp1list, p0=[0, 0, 0, 0], maxfev=5000
                            )
                            [c0, c1, c2, c3] = opt
                            expected = PRESEXPECTED(
                                dp1list, presmpi_list, c0, c1, c2, c3
                            )
                            Epres = []
                            ...
                            if (
                                abs(mu) < 1 and c2 > 0
                            ):  # otherwise: the fit didn't go as planned: large deviation from expected values..
            ...
            print(
                str(count)
                + " fields out of "
                + str(len(latbins1) * len(lonbins1))
                + " bins do not have a fit"
            )
            ...
            while var != 0:
                ...
                zeroeslist = [
                    [i1, j1]
                    for i1, x in enumerate(matrix_mean)
            ...
            while var != 0:
                ...
                for [i, j] in zeroeslist:
                    ...
                    for item in lijst:
                        (i0, j0) = item
                        if (
                            math.isnan(matrix_mpi[i0, j0]) == False
