...
def TC_pressure(
    ...
    for storm_number, month, latfull, lonfull, landfallfull in zip(
        range(0, int(storms)), genesis_months, latlist, lonlist, landfalllist
    ):
        i = 0
        vmax = 0
        count = 0
        ...
        p_threshold = min(constants_pressure[:, 6]) - 10.0
            ...
            if (
                ...
                if (
                    ...
                    if (p < p_threshold) | math.isnan(p):
                        ...
                        epsilon = rng.normal(EPmu, EPstd)
                        dp0 = float(y + epsilon)

                        while (
                            dp0 < dpmin
                        ):  # if more intensification than seen in the underlying dataset
                            if (
                                y - dpmin > EPmu - 2.0 * EPstd
                            ):  # epsilon should be resampled
                                epsilon = rng.normal(EPmu, EPstd)
                                dp0 = y + epsilon
                            else:  # y is already smaller than dpmin.
                                dp0 = rng.uniform(dpmin, 0)

                        while (
                            dp0 > dpmax
                        ):  # if more weakening than seen in the underlying dataset
                            if y - dpmax < EPmu + 2.0 * EPstd:
                                epsilon = rng.normal(EPmu, EPstd)
                                dp0 = y + epsilon
                            else:
                                dp0 = rng.uniform(0, dpmax)
