...
for i, prediction in enumerate(model_predictions):
    print(
        "Prediction diff", np.mean(((model_predictions[0] - model_predictions[i])) ** 2)
    )
    ...
    plotLatLonGridData(np.sum(real_outputs[i], axis=2), 0.5)
    plotLatLonGridData(np.sum(real_outputs[i] - real_outputs[0], axis=2), 0.5)
