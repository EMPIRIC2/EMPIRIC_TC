...
def TC_variables():
    ...
    poisson = {
        i: [0] for i in range(0, 6)
    }  # Poisson genesis parameters (avg number of TC formations per year)
    genesis_poisson = []
    ...
    for idx in range(0, 6):
        ...
        genesis_dpres[idx] = {
            i: [] for i in monthsall[idx]
        }  # genesis change in pressure
        genesis_pres_var[idx] = {
            i: [] for i in monthsall[idx]
        }  # genesis variables for pressure.
    ...
    for i in range(len(latlist)):
        if len(latlist[i]) > 0:
            idx = basinlist[i][
                0
            ]  # this is the index for each of the basins. 0=EP, 1=NA, 2=NI, 3=SI, 4=SP, 5=WP
    ...
    for idx in range(0, 6):
        genesis_poisson.append(round(poisson[idx][0] / 38.0, 1))
