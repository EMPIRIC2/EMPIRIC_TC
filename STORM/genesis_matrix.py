...
def Dataframe_Genesis_Basin(idx, month):
    ...
    genesis = np.load(
        os.path.join(__location__, "GEN_LOC.npy"), allow_pickle=True
    ).item()
    ...
    for i in range(len(lat_init_list)):
        if (
            lat_init_list[i] <= lat1
            and lat_init_list[i] >= lat0
            and lon_init_list[i] <= lon1
            and lon_init_list[i] >= lon0
        ):
...
def Makegrid(idx, month):
    ...
    # ==============================================================================
    df = Dataframe_Genesis_Basin(
        idx, month
    )  # generate dataframe for genesis counts (step 2)
    step = 5.0  # define grid size of initial data binning
    ...
    np.savetxt(
        os.path.join(
            __location__,
            "COUNTS_GENESIS_MATRIX_" + str(idx) + "_" + str(month) + ".txt",
        ),
        count_matrix,
    )
    np.savetxt(
        os.path.join(
            __location__, "GRID_GENESIS_MATRIX_" + str(idx) + "_" + str(month) + ".txt"
        ),
        grid,
    )
    return grid
